...
[Rogue EP/COOP Exception MACs missing][d30]     | CSCwp64296   | :white_check_mark: | :no_entry_sign:
...
[d30]: #rogue-epcoop-exception-macs-missing
...
Due to the defect [CSCwp64296][64], rogue endpoint (EP) and COOP exception MAC address configurations may be lost after a stateless reload of spine switches in an ACI fabric. The `presListener` MO, which holds the exception list configuration for the tenant shard, is missing or incomplete on the APIC side. This leads to spine switches not receiving the `rogueBDDef` configuration after reload.
